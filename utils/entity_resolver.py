...
from .entity_normalization import normalize_entity_name, are_similar_entities, get_canonical_name
...
class EntityResolver:
    ...
    def find_duplicate_entities(self, entity_type: str = None, threshold: float = 0.85) -> List[Tuple[str, str, float]]:
        """
    ...
    def merge_entities(self, entity_id_1: str, entity_id_2: str, keep_canonical: bool = True) -> bool:
        """
        ...
        with self.driver.session() as session:
            ...
            try:
                ...
                for record in outgoing:
                    ...
                    """, id1=entity_id_1, target_id=target_id, rel_type=rel_type)
                    ...
                    if exists_record and not exists_record["exists"]:
                        ...
                        """, id1=entity_id_1, target_id=target_id, props=props)
                ...
                for record in incoming:
                    ...
                    exists = session.run("""
                        ...
                        WHERE type(r) = $rel_type
                        RETURN count(r) > 0 as exists
                    """, source_id=source_id, id1=entity_id_1, rel_type=rel_type)
                    ...
                    if exists_record and not exists_record["exists"]:
                        ...
                        session.run(f"""
                            ...
                            SET r = $props, r.created_at = timestamp()
                        """, source_id=source_id, id1=entity_id_1, props=props)
    ...
    def merge_all_duplicates(self, entity_type: str = None, threshold: float = 0.85, dry_run: bool = False) -> Dict:
        """
        ...
        for entity_id_1, entity_id_2, similarity in duplicates:
            if self.merge_entities(entity_id_1, entity_id_2):
                merged_count += 1
                print(f"  ✓ Merged: {entity_id_1} <-> {entity_id_2} (similarity: {similarity:.2f})")
            else:
                failed_count += 1
