...
class CoreferenceResolver:
        ...
        for pattern, replacement_func in patterns:
            ...
            for match in reversed(matches):
                ...
                if replacement:
                    ...
                    resolved_text = (
                        resolved_text[:start] + replacement + resolved_text[end:]
                    )

        return resolved_text

    def _find_last_entity(
        self, entities: List[Dict], entity_type: Optional[str] = None
    ) -> Optional[str]:
        """
        ...
        if entity_type:
            filtered = [
                e for e in entities if e.get("type", "").lower() == entity_type.lower()
            ]
            if filtered:
    ...
    def resolve_pronouns_in_sentence(
        self, sentence: str, previous_sentences: List[str], entities: List[Dict]
    ) -> str:
        """
        ...
        pronouns = {
            ...
            "they": self._find_last_entity(entities, "company")
            or self._find_last_entity(entities),
