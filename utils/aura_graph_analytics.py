...
class AuraGraphAnalytics:
    ...
    def detect_communities(
        ...
        try:
            ...
            print(
                f"   ✅ Projected graph: {result.nodeCount} nodes, {result.relationshipCount} relationships"
            )
        ...
        try:
            ...
            print(
                f"   ✅ Found {len(filtered_communities)} communities (min size: {min_community_size})"
            )
            ...
            try:
                ...
                print(
                    f"   ✅ Wrote {sum(len(e) for e in filtered_communities.values())} community IDs to database"
                )
        ...
        try:
            with driver.session() as session:
                ...
                for community_id, entity_names in communities.items():
                    ...
                    """
                    session.run(
                        query, entity_names=entity_names, community_id=community_id
                    )
        finally:
            driver.close()
