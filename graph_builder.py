...
class TechCrunchGraphBuilder:
    ...
    def create_relationship(self, relationship: Dict, article_id: str):
        ...
        if rel_type == "MENTIONED_IN":
            print(
                f"  ⚠️  WARNING: Attempted to create MENTIONED_IN relationship from {source_name} to {target_name}. Skipped."
            )
            return
        ...
        if is_techcrunch_related(source_name) or is_techcrunch_related(target_name):
            print(
                f"  ⚠️  WARNING: Attempted to create relationship involving TechCrunch/Disrupt entity: {source_name} -> {target_name}. Skipped."
            )
            return
        ...
        if rel_type not in valid_types:
            print(
                f"  ⚠️  WARNING: Invalid relationship type '{rel_type}' from {source_name} to {target_name}. Skipped."
            )
            return
        ...
        with self.driver.session() as session:
            ...
            if rel_type == "MENTIONED_IN":
                print(
                    f"  ❌ ERROR: MENTIONED_IN relationship reached create_relationship. This should never happen!"
                )
                return
    ...
    def ingest_extraction(self, extraction: Dict, skip_if_exists: bool = True):
        ...
        if skipped_mentioned > 0:
            print(
                f"  ⚠️  Skipped {skipped_mentioned} MENTIONED_IN relationships (use properties instead)"
            )
        if skipped_techcrunch > 0:
            print(
                f"  ⚠️  Skipped {skipped_techcrunch} TechCrunch/Disrupt related relationships"
            )
    ...
    def enrich_all_companies(self, enriched_companies: Dict[str, Dict]):
        ...
        for company_name, enriched_data in enriched_companies.items():
            try:
                ...
                if success:
                    ...
                    confidence = enriched_data.get("confidence_score", 0.0)
                    print(
                        f"  ✓ Enriched: {company_name} (confidence: {confidence:.2f})"
                    )
                else:
                    failed_count += 1
            except Exception as e:
        ...
        print("\n" + "=" * 80)
        print(
            f"✅ ENRICHMENT COMPLETE: {enriched_count} companies enriched, {failed_count} failed"
        )
        print("=" * 80 + "\n")
