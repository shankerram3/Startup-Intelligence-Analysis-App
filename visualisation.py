...
with driver.session() as s:
    ...
    if community_count == 0:
        ...
        result = detector.detect_communities(
            algorithm="label_propagation", min_community_size=3
        )
        print(
            f"✓ Community detection complete: {result.get('total_communities', 0)} communities found (min_size >= 3)"
        )
...
for n in nodes:
    net.add_node(
        n["id"],
        label=f'{n["name"]} ({n["label"]})',
        color=color(n["cid"]),
        title=f'Community {n["cid"]}',
    )
for e in edges:
