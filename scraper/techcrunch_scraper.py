...
class TechCrunchScraper:
    ...
    async def discover_articles(
        self, 
        category_url: str = "https://techcrunch.com/category/startups/"
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            while current_url and (self.max_pages is None or page_num <= self.max_pages):
        ...
        discovery_file = self.metadata_dir / f"discovered_articles_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(discovery_file, 'w', encoding='utf-8') as f:
            json.dump([article.dict() for article in self.discovered_articles], f, indent=2, ensure_ascii=False)
    ...
    def _extract_articles_from_page(self, soup: BeautifulSoup, page_num: int) -> List[ArticleMetadata]:
    ...
    async def extract_articles(
        self, 
        articles: Optional[List[ArticleMetadata]] = None,
        batch_size: int = 10
    ):
        """
        ...
        # Save failed URLs
        if self.failed_urls:
            failed_file = self.metadata_dir / f"failed_articles_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def _extract_single_article(
        ...
        try:
            ...
            headline = headline_elem.get_text(strip=True) if headline_elem else article_meta.title
            ...
            for ad in content_div.find_all(['div', 'aside'], class_=re.compile('ad-unit|ad-slot')):
                ad.decompose()
            ...
            author_elem = soup.find('a', class_='loop-card__author') or soup.find('a', href=re.compile('/author/'))
            if author_elem:
    ...
    def _save_article(self, article: ArticleContent):
        ...
        if article.published_date:
            try:
                ...
                date_dir = self.articles_dir / date_obj.strftime('%Y-%m') / date_obj.strftime('%d')
            except:
    ...
    def _print_final_stats(self):
        ...
        if self.stats['articles_discovered'] > 0:
            success_rate = (self.stats['articles_extracted'] / self.stats['articles_discovered']) * 100
        ...
        # Save final stats
        stats_file = self.metadata_dir / f"scraping_stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
