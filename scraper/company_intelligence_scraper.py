...
class CompanyIntelligenceScraper:
    ...
    async def scrape_company(
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                intelligence['extracted_data'] = self._aggregate_intelligence(intelligence['pages_scraped'])
    ...
    async def _scrape_page(
        ...
        try:
            ...
            if 'about' in path or 'company' in path or path == '/':
                data['structured_data']['about'] = await self._extract_about_info(page, data['text_content'])

            if 'team' in path or 'people' in path or 'about' in path:
                data['structured_data']['team'] = await self._extract_team_info(page, data['text_content'])

            if 'press' in path or 'news' in path or 'blog' in path:
                data['structured_data']['news'] = await self._extract_news_info(page, data['text_content'])

            if 'product' in path or 'solution' in path or path == '/':
                data['structured_data']['products'] = await self._extract_product_info(page, data['text_content'])

            if 'pricing' in path:
                data['structured_data']['pricing'] = await self._extract_pricing_info(page, data['text_content'])
    ...
    async def _extract_team_info(self, page: Page, text: str) -> Dict:
        ...
        try:
            ...
            for member in team_members[:10]:  # Limit to top 10
                ...
                if any(keyword in title_lower for keyword in ['founder', 'co-founder', 'founding']):
                    info['founders'].append(member['name'])
                elif any(keyword in title_lower for keyword in ['ceo', 'cto', 'cfo', 'coo', 'chief', 'president', 'vp']):
    ...
    async def _extract_product_info(self, page: Page, text: str) -> Dict:
        ...
        tech_keywords = [
            ...
            'blockchain', 'crypto', 'web3', 'cloud', 'SaaS',
            'mobile', 'iOS', 'Android', 'API', 'platform',
    ...
    async def _extract_pricing_info(self, page: Page, text: str) -> Dict:
        ...
        if re.search(r'\bfree\b', text, re.IGNORECASE):
            ...
            info['pricing_model'] = 'freemium' if re.search(r'\bpaid\b|\bpremium\b|\bpro\b', text, re.IGNORECASE) else 'free'
    ...
    def _aggregate_intelligence(self, pages_scraped: Dict) -> Dict:
        ...
        for page_key, page_info in pages_scraped.items():
            ...
            if 'news' in structured:
                ...
                aggregated['funding_announcements'].extend(news.get('recent_announcements', []))
        ...
        aggregated['executives'] = list({exec['name']: exec for exec in aggregated['executives']}.values())
        aggregated['technologies'] = list(set(aggregated['technologies']))

        return aggregated

    def _save_intelligence(self, company_name: str, article_id: str, intelligence: Dict):
    ...
    async def scrape_companies_batch(
        ...
        logger.info(f"Scraping intelligence for {len(companies_to_scrape)} companies from article {article_id}")
