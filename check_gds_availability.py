...
def check_gds_availability():
    ...
    try:
        ...
        with driver.session() as session:
            ...
            print("-" * 80)
            try:
                result = session.run("CALL dbms.components() YIELD name, versions, edition RETURN name, versions[0] as version, edition LIMIT 1")
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            test_result = session.run("CALL gds.version() YIELD version RETURN version")
                            test_record = test_result.single()
                            if test_record:
                                gds_count = 1  # GDS is available if version() works
                                print(f"   ✅ GDS library is AVAILABLE (via version check)")
                                print(f"   GDS Version: {test_record.get('version', 'unknown')}")
                        except Exception:
                            gds_count = 0
                
                if gds_count > 0:
                    if not test_record:  # Only print if we haven't already printed from version check
                    ...
                    try:
                        ...
                        if proc_record and proc_record.get("sample_procedures"):
                            print(f"   Sample procedures: {', '.join(proc_record['sample_procedures'])}")
                    except Exception:
                        pass
                else:
                    ...
                    if is_aura:
                        ...
                        print("      • Available on Aura Professional ($65+/month) and Enterprise")
                        print("      • Not included in Aura Free tier")
                        print("\n   💡 Solutions:")
                        print("      1. Upgrade to Aura Professional tier (if needed)")
                        print("      2. Use the fallback simple community detection (already working)")
                        print("      3. Use local Neo4j with GDS plugin for development")
            except Exception as e:
            ...
            print("-" * 80)
            try:
                graphs_result = session.run("CALL gds.graph.list() YIELD graphName RETURN collect(graphName) as graphs")
            ...
            if not gds_available:
                try:
                    list_result = session.run("CALL gds.list() YIELD name RETURN count(*) as count")
                    list_record = list_result.single()
                    if list_record and list_record.get('count', 0) >= 0:  # Just check if it doesn't error
                        gds_available = True
            ...
            if not gds_available:
                if is_aura:
                    ...
                    try:
                        ed_result = session.run("CALL dbms.components() YIELD edition RETURN edition LIMIT 1")
                        edition_record = ed_result.single()
                        edition = edition_record.get('edition', 'unknown') if edition_record else 'unknown'
                        
                        if edition.lower() in ['enterprise', 'enterprise edition']:
                            print("   ⚠️  You're on Aura Enterprise, but GDS procedures are not available.")
                            print("   This is expected! On Aura, GDS works differently:")
                            print("     • GDS is NOT installed directly on AuraDB instances")
                            print("     • Instead, use Aura Graph Analytics (serverless GDS service)")
                            print("     • See: https://neo4j.com/docs/graph-data-science-client/current/graph-analytics-serverless/")
                            print("\n   To use GDS on Aura:")
                            ...
                            print("   The pipeline will use simple community detection (which works fine).")
                            print("\n   To enable GDS:")
                            print("   • Upgrade to Aura Professional tier in https://console.neo4j.io/")
                    except Exception:
                    ...
                    print("   1. Download from: https://neo4j.com/docs/graph-data-science/")
                    print("   2. Place in plugins directory")
                    print("   3. Add to neo4j.conf: dbms.security.procedures.unrestricted=gds.*")
